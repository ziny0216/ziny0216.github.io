...
export const allProject = [
  {
    ...
    troubleShooting:
      '아이큐 비타민 담당 프론트엔드에게 맞춰 ui를 구성하여 중첩 라우팅을 사용하지 않았다. 쿼리스트링의 type을 참조하여 if문을 통해 컴포넌트 렌더링 되도록 작업했다.<br/>내가 다시 투입되어 개발까지 해야했을때 거의 동일한 ui 임에도 중첩 라우팅의 부재로 컴포넌트 마다 무한스크롤 , 탭 클릭시 api 호출 중복 코드가 발생하였다. <br/>중복코드를 제거하기 위해 custom hook에서 type 값을 받아 switch 문으로 api 호출 분기를 태웠고 그 이후 예외처리와 무한스크롤의 공통화했다.<br/> 이렇게 만든 custom hook은 리스팅 페이지에 전반적으로 공통 사용 되었다. ',
    medias: [
      {
        src: iq_login,
        type: 'video',
  ...
  },
  {
    ...
    troubleShooting:
      '데이페이와 자매사이트로 영상장비 판매 사이트이다.추가되는 기능의 api를 제외하고는  데이페이와 동일한 api 구조를 가기로 백엔드 개발자와 협의하였다.<br/> 기존 데이페이의 50% 개발 , 100% ui 를 시네몰에 입혔고 시네몰의 디자인을 입혀야했다. 이미 개발 및 ui 는 만들어져있어 수정하는데 어려움을 겪었고 최대한 기존 데이터 할당을 유지한채로 작업했다. 브랜드와 카테고리는 db 설계가 따로 되어있어 데이터를 따로 받아오는 와중에 카테고리는 3depth 까지 있다.<br/>그래도 새로 만드는것 보다 기존의 형태에서 ui 만 바꾸는것이 합리적이고 적절한 선택이었다. 시네몰 작업 이후 데이페이도 시네몰 카테고리와 유사한 ui 형태로 변경되었다. ',
    medias: [
      {
        src: cimemall_main,
        type: 'video',
